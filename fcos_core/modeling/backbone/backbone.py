...
def build_resnet_backbone(cfg, basemodel=None):
...
def build_resnet_fpn_p3p7_backbone(cfg, basemodel=None):
    body = resnet.ResNet(cfg) if basemodel is None else basemodel
    in_channels_stage2 = cfg.MODEL.RESNETS.RES2_OUT_CHANNELS
    out_channels = cfg.MODEL.RESNETS.BACKBONE_OUT_CHANNELS
...
def build_custom_basemodel_backbone(cfg, basemodel):
    assert cfg.MODEL.BACKBONE.CONV_BODY in registry.BACKBONES, \
        "cfg.MODEL.BACKBONE.CONV_BODY: {} are not registered in registry".format(
            cfg.MODEL.BACKBONE.CONV_BODY
        )
    return registry.BACKBONES[cfg.MODEL.BACKBONE.CONV_BODY](cfg, basemodel)
